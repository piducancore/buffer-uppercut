...
/// Notify hosts that a synchronous state or preset load changed parameter
/// values. CLAP requires this rescan for host-side parameter caches to observe
/// the values written by `state::apply_params`.
unsafe fn notify_param_values_changed<P: PluginExport>(data: &ClapPluginData<P>) {
    unsafe {
        if !data.host.is_null()
            && !data.host_params.is_null()
            && let Some(rescan) = (*data.host_params).rescan
...
unsafe extern "C" fn state_load<P: PluginExport>(
    ...
    run_extern_callback_with::<P, bool>("CLAP", "load_state", false, || unsafe {
        ...
        notify_param_values_changed(data);
