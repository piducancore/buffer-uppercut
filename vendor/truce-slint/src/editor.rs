...
pub struct SlintEditor<P: Params + ?Sized> {
    ...
    can_maximize: bool,
    /// Whether dispatched keyboard events should continue to the parent
...
impl<P: Params + 'static> SlintEditor<P> {
    ...
    /// Allow the parent host to process keyboard events after Slint.
    ///
    /// Events are still dispatched to Slint first, so focused controls keep
    /// receiving key presses. When enabled the baseview handler reports the
    /// event as ignored afterward, allowing DAW shortcuts and virtual MIDI
    /// keyboards to see the same key. The default is `false`.
    #[must_use]
...
impl<P: Params + ?Sized + 'static> WindowHandler for SlintWindowHandler<P> {
    // This is upstream TRUCE's frame pump plus the narrow keyboard policy
    // patch; keep its control flow intact so the vendored diff stays auditable.
    #[allow(clippy::too_many_lines)]
    fn on_frame(&mut self, window: &mut Window) {
...
/// Status returned after a mapped keyboard event has been delivered to Slint.
///
...
#[cfg(test)]
mod tests {
    use super::{EventStatus, dispatched_keyboard_status};
