        ...
        let event_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            match event {
                ...
                Event::Keyboard(kb) => {
                    ...
                    let wants_keyboard_input = self.egui_ctx.egui_wants_keyboard_input();
                    ...
                    keyboard_event_status(wants_keyboard_input)
                }
                Event::Window(win) => {
...
fn keyboard_event_status(wants_keyboard_input: bool) -> EventStatus {
    if wants_keyboard_input {
        EventStatus::Captured
    } else {
        // A plugin editor with no focused text widget must return keyboard
        // events to its parent. DAWs such as REAPER use those unconsumed
        // events to drive the virtual MIDI keyboard.
        EventStatus::Ignored
    }
}
...
mod keyboard_event_tests {
    use super::{EventStatus, keyboard_event_status};

    #[test]
    fn keyboard_events_bubble_to_the_daw_without_text_focus() {
        assert_eq!(keyboard_event_status(false), EventStatus::Ignored);
        assert_eq!(keyboard_event_status(true), EventStatus::Captured);
    }
}
