...
pub trait EditorBridge: Send + Sync {
    /// Notify the host that non-parameter persistent editor state changed.
    /// Wrappers marshal the notification to the host main thread. This is
    /// not a state load and must not be called from the audio process path.
...
// Decorate rather than adding a required ClosureBridge field: downstream
// standalone and other format wrappers keep their existing struct literals.
struct StateDirtyBridge {
    inner: Arc<dyn EditorBridge>,
    notify: Box<dyn Fn() + Send + Sync>,
}

impl EditorBridge for StateDirtyBridge {
    fn mark_state_dirty(&self) {
        (self.notify)();
    }
    fn begin_edit(&self, id: u32) {
    ...
    fn request_resize(&self, w: u32, h: u32) -> bool {
        self.inner.request_resize(w, h)
    }
    fn get_param(&self, id: u32) -> f64 {
        self.inner.get_param(id)
    }
    fn get_param_plain(&self, id: u32) -> f64 {
        self.inner.get_param_plain(id)
    }
    fn format_param(&self, id: u32) -> String {
        self.inner.format_param(id)
    }
    fn format_param_into(&self, id: u32, out: &mut String) {
        self.inner.format_param_into(id, out);
    }
    fn get_meter(&self, id: u32) -> f32 {
        self.inner.get_meter(id)
    }
    fn get_state(&self) -> Vec<u8> {
        self.inner.get_state()
    }
    fn set_state(&self, data: Vec<u8>) {
        self.inner.set_state(data);
    }
    fn transport(&self) -> Option<TransportInfo> {
...
impl<P: ?Sized> PluginContext<P> {
    /// Install a format-specific persistent-state change notification.
    #[must_use]
    pub fn with_state_dirty_callback(mut self, notify: impl Fn() + Send + Sync + 'static) -> Self {
        self.bridge = Arc::new(StateDirtyBridge {
            inner: self.bridge,
    ...
    /// Notify the host after editing persistent non-parameter state.
    pub fn mark_state_dirty(&self) {
        self.bridge.mark_state_dirty();
    }
...
#[cfg(test)]
mod state_dirty_tests {
    use super::{ClosureBridge, PluginContext};
    use std::sync::{
        Arc,
    ...
    #[test]
    fn dirty_callback_survives_context_retyping_and_bridge_clone() {
        let bridge = ClosureBridge {
            begin_edit: Box::new(|_| {}),
            set_param: Box::new(|_, _| {}),
            end_edit: Box::new(|_| {}),
            request_resize: Box::new(|_, _| false),
            get_param: Box::new(|_| 0.25),
            get_param_plain: Box::new(|_| 42.0),
            format_param: Box::new(|_| "42".into()),
            get_meter: Box::new(|_| 0.0),
            get_state: Box::new(Vec::new),
            set_state: Box::new(|_| {}),
            transport: Box::new(|| None),
        };
        let context = PluginContext::new(Arc::new(bridge), Arc::new(()));
        // Default is harmless for standalone/third-party wrappers.
        context.mark_state_dirty();
        let calls = Arc::new(AtomicU32::new(0));
        let observed = calls.clone();
        let context = context.with_state_dirty_callback(move || {
            observed.fetch_add(1, Ordering::Relaxed);
        });
        context.mark_state_dirty();
        context.with_params(Arc::new(1u32)).mark_state_dirty();
        let bridge = context.bridge().clone();
        bridge.mark_state_dirty();
        assert_eq!(calls.load(Ordering::Relaxed), 3);
        assert_eq!(bridge.get_param(1), 0.25);
        assert_eq!(bridge.get_param_plain(1), 42.0);
    }
}
